"""EC problem 还未解决,不够快(先提前算了,memo了一手...)"""
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def about1(item):
        lst = split(lower(remove_punctuation(item)))
        for i in lst :
            if i in subject:
                return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    length_typed , length_source = len(typed_words) , len(source_words)
    if length_source==0 and length_typed==0:
        return 100.0
    if length_typed==0 or length_source==0:
        return 0.0
    cnt=0
    for i in range(min(len(source_words),len(typed_words))):
        if typed_words[i]==source_words[i]:
            cnt+=1
    return cnt/length_typed*100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    lst = [limit-diff_function(typed_word, source_word, limit) for source_word in word_list]
    if max(lst)<0:
        return typed_word
    return word_list[lst.index(max(lst))]
...
def feline_fixes(typed, source, limit):
    ...
    return cnt + feline_fixes(typed[1:],source[1:],limit)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit<0 or (len(typed)==0 and len(source)==0) or typed==source:  
        return 0 
    if len(typed)==0 or len(source)==0:
        return abs(len(typed)- len(source))
    if typed[0]==source[0]:
...
def time_per_word(words, times_per_player):
    ...
    for times in times_per_player:
        time.append([times[i+1]- times[i] for i in range(len(times)-1)])
...
def fastest_words(match):
    ...
    "*** YOUR CODE HERE ***"
    lst = []
    for id in range(len(get_all_times(match))):
        lst.append([])

    for word_index in range(len(get_all_words(match))):
        box = [time(match, player_id, word_index) for player_id in range(len(get_all_times(match)))]
        find = box.index(min(box))
        lst[find].append(get_word(match, word_index))
