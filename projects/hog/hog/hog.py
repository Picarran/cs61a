...
def play(strategy0, strategy1, update,
    ...
    while (score0 <goal and score1<goal):
        ...
        if (not (score0 <goal and score1<goal)):
            break
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(0,goal+1):
        for op_score in range(0,goal+1):
            if strategy(score,op_score)!=rolls:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max ,max_index= 0,1
    for i in range(1,11):
        res=make_averaged(roll_dice,total_samples)(i,dice)
        if res > max:
            max =res
            max_index=i
    return max_index
...
def final_strategy(score, opponent_score,dice=six_sided):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    def check():
        for i in range(0,3):
            if fuzzy_update(i,score,opponent_score,dice)>=100:
                return i
        return -1
    ...
    if flag>=0:
        ...
        # return fuzzy_strategy(score,opponent_score,make_averaged(roll_dice,1)(6,dice),max_scoring_num_rolls(dice,1))
        return fuzzy_strategy(score,opponent_score,make_averaged(roll_dice,1)(6,dice),6)
