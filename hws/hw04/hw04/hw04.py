...
def totals_tree(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    if is_mobile(m) or is_planet(m):
        label = total_weight(m)
        branches = []
        if is_mobile(m):
            l = end(left(m))
            r = end(right(m))
            branches.append(totals_tree(l))
            branches.append(totals_tree(r))
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    for bs in branches(t):
        brs.append(replace_loki_at_leaf(bs,lokis_replacement))
    return tree(new_label,brs)
